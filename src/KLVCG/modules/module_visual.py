...
WEIGHTS_NAME = 'single_pytorch_model.bin'
...
class VisualEmbeddings(nn.Module):
    ...
    def __init__(self, config, position_embeddings):
        ...
        # 这里的vocab size已经不是传统意义的vocab size了，应该是特征维度的意思，把提取好的特征也看成one-hot的标识再来一次
        # 所以这里不用nn.Embeddings了，因为embeddings只能进编号id，不能进one-hot向量，输入维度应该是[batch_size, seq_len]而不应该是[batch_size, seq_len, vocab_size]
    ...
    def forward(self, input_embeddings):
        ...
        words_embeddings = input_embeddings
...
class VisualModel(PreTrainedModel):
    ...
    def __init__(self, config, position_embeddings):
        ...
        self.embeddings = VisualEmbeddings(config, position_embeddings)
    ...
    def forward(self, video, attention_mask=None, output_all_encoded_layers=True):
        # video [batch_size, seq_len, 1024]
        if attention_mask is None:
            attention_mask = torch.ones(video.size(0), video.size(1)).to(video.device) # [batch_size, seq_len]
        ...
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2) # [batch_size, 1, 1, time_step]
        ...
        sequence_output = encoded_layers[-1] # 取最后一个block作为输出
