...
class CrossEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.loc_type_embeddings = nn.Embedding(5, config.hidden_size)
        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = LayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, concat_embeddings, concat_type=None, loc_type=None):
        token_type_embeddings = self.token_type_embeddings(concat_type)
        loc_type_embeddings = self.loc_type_embeddings(loc_type)
        embeddings = concat_embeddings + token_type_embeddings + loc_type_embeddings
...
class CrossModel(PreTrainedModel):
    ...
    def forward(self, concat_input, concat_type=None, attention_mask=None, output_all_encoded_layers=True, loc_type=None):
        extended_attention_mask = attention_mask.unsqueeze(1)
        extended_attention_mask = extended_attention_mask.to(dtype=self.dtype) # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0

        embedding_output = self.embeddings(concat_input, concat_type, loc_type)
