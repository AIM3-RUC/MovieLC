...
CONFIG_NAME = 'kg_config.json'
WEIGHTS_NAME = 'pytorch_model.bin'


class KGConfig(PretrainedConfig):
...
class BertAttention(nn.Module):
    ...
    def forward(self, q, k, v, attention_mask):
        ...
        att_output = self.att(q, k, v, attention_mask)
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.slf_attn = BertAttention(config)
        self.enc_attn = BertAttention(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)

    def forward(self, hidden_states, attention_mask, q_input, query_attention_mask):
        slf_output = self.slf_attn(hidden_states, hidden_states, hidden_states, attention_mask) # transformer encoder图中的第一个sublayer
        attention_output = self.enc_attn(q_input, slf_output, slf_output, query_attention_mask)
...
class BertEncoder(nn.Module): # 由多个attention layer构成
    ...
    def forward(self, hidden_states, attention_mask, q_input, query_attention_mask, output_all_encoded_layers=True):
        all_encoder_layers = []
        for layer_module in self.layer:
            # input output均为 [batch_size, seq_len, hidden_size]
            hidden_states = layer_module(hidden_states, attention_mask, q_input, query_attention_mask)
...
class KGModel(PreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(KGModel, self).__init__(config)
    ...
    def forward(self, input_ids, pos_ids=None, attention_mask=None, q_input=None, output_all_encoded_layers=True):
    ...
    # from_sequence是作为query，to_sequence是作为key和value，当两者相同时即为self-attention，这里就是self-attention
        # K_token_ids [bs, max_words] K_pos_ids [bs, max_words] K_attn_mask [bs, max_words, max_words]
        # q_input [bs, time, h]
        # 1.1 input_ids [batch_size, seq_len] （要从[batch_size, time_step, max_words]变为[batch_size*time_step, max_words）
        time_len = q_input.size(1)

        extended_attention_mask = attention_mask.unsqueeze(1) # [batch_size, 1, to_seq_length, to_seq_length]
        extended_attention_mask = extended_attention_mask.to(dtype=self.dtype) # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0 #  [batch_size, 1, to_seq_len, to_seq_length]
        query_attention_mask = extended_attention_mask[:, :, :time_len]
