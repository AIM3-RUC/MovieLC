...
class DecoderConfig(PretrainedConfig):
    ...
    def __init__(self,
        ...
        Args:
            ...
            num_hidden_layers: Number of hidden layers in the Transformer encoder. 在这里其实是无效参数
            ...
            num_decoder_layers: 替代了num_hidden_layers
        """
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config, decoder_model_embedding_weights):
        ...
        '''
        这里就是为了把attention好的decoder输出[batch_size, tgt_len, h]转换成词语预测[batch_size, tgt_len, vocab_size]
        之后再vocab_size上每个分量就代表每个词语的置信度，最大的那个就是当前step对应的输出词语。出来后和GT的one-hot vocab_size
        大小的向量算CE Loss就可以完成caption生成的训练过程
        '''
        self.transform = BertPredictionHeadTransform(config)

        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        # a=nn.embedding(A,B), a.weight.size()=(A,B)
        # b=nn.embedding(A,B), b.weight.size()=(B,A)
        # 下面这里相当于nn.linear(hidden_size, vocab_size)
        self.decoder = nn.Linear(decoder_model_embedding_weights.size(1),
                                 decoder_model_embedding_weights.size(0),
                                 bias=False)
        # 这样引入参数是非常合理的非常好的设计，因为nn.embedding对于一个token，说是查表，也可以看成是x[1,30005]*W[30005,1024]=h[1,1024]
        # x是一个one-hot的话，就相当于选择W的某一行。而在这里，我们有h[1,1024],去和转置的W[1024,30005]相乘，相当于计算h和W中30005个词的特征的相似度
        # 从而得到其在每个词上的概率
        self.decoder.weight = decoder_model_embedding_weights
        # nn.parameter 将一个不可训练的类型Tensor转换成可以训练的类型parameter并将这个parameter绑定到这个module
        self.bias = nn.Parameter(torch.zeros(decoder_model_embedding_weights.size(0)))

    def forward(self, hidden_states):
        hidden_states = self.transform(hidden_states)
        hidden_states = self.decoder(hidden_states) + self.bias
        # 输出为[batch_size, tgt_len, vocab_size]
        return hidden_states
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_output, slf_attn_mask=None, dec_enc_attn_mask=None):
        # dec_input [batch_size, tgt_len, h]
        # enc_output [batch_size, src_len, h]
        # self_attn_mask [batch_size, 1, tgt_len, tgt_len]
        # attention_mask [batch_size, 1, 1, src_len]
        # slf_output [batch_size, tgt_len, h] 这一步是对过去已生成token的self-attention融合优化token特征，所以对角线的attention当然是要保留的
        slf_output, _ = self.slf_attn(dec_input, dec_input, dec_input, slf_attn_mask)
        # dec_output 已生成token信息的slf_output作为query，enc_output作为key和value
        # dec_output [batch_size, tgt_len, h] dec_att_scores [batch_size, tgt_len, src_len]
...
class DecoderEmbeddings(nn.Module):
    ...
    def __init__(self, config, decoder_word_embeddings_weight, decoder_position_embeddings_weight):
        ...
        self.word_embeddings.weight = decoder_word_embeddings_weight # nn.embedding这就相当于一个被查表的字典W，weight就是W的所有参数了，它本质上相当于nn.Linear bias=0的版本
...
class DecoderModel(PreTrainedModel):
    ...
    def __init__(self, config, decoder_word_embeddings_weight, decoder_position_embeddings_weight):
        ...
        self.config = config
        self.max_target_length = config.max_target_embeddings # decoder output sentence len
    ...
    def forward(self, input_ids, encoder_outs=None, answer_mask=None, encoder_mask=None):
        ...
        Args:
            ...
            encoder_outs (Tensor, optional): output from the cross encoder, used for encoder-side attention
            answer_mask: 生成时，只能看到句子前面的单词，后面的看不到，这里做一个三角形的遮掩
        Returns:
            tuple:
        ...
        """
        batch_size, src_len = encoder_outs.size()[0:2] # src_len: cross encoder output seq_len
        tgt_len = self.max_target_length # tgt_len: decoder output sentence len

        embedding_output = self.embeddings(input_ids)
    
        extended_encoder_mask = encoder_mask.unsqueeze(1)   # b x 1 x 1 x ls
        ...
        extended_answer_mask = answer_mask.unsqueeze(1)
        extended_answer_mask = extended_answer_mask.to(dtype=self.dtype)  # fp16 compatibility
        extended_answer_mask = (1.0 - extended_answer_mask) * -10000.0

        decoded_layers, dec_att_scores = self.decoder(embedding_output,
                                      encoder_outs,
                                      extended_answer_mask,
                                      extended_encoder_mask,
                                      )
        sequence_output = decoded_layers[-1] # [batch_size, tgt_len, h]
