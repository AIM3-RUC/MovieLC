...
CONFIG_NAME = 'danmaku_config.json'
WEIGHTS_NAME = 'pytorch_model.bin'


class DanmakuConfig(PretrainedConfig):
...
class BertEmbeddings(nn.Module):
    ...
    """
    def __init__(self, config, position_embeddings):
    ...
    def forward(self, input_ids, pos_id=None):
        batch_size, seq_length, h = input_ids.size() # input_ids [bs, seq_len]
        words_embeddings = input_ids

        position_ids = torch.cat((torch.arange(start=0, end=int(seq_length/2), dtype=torch.long, device=input_ids.device), torch.arange(start=int((seq_length/2)+1), end=int(seq_length+1), dtype=torch.long, device=input_ids.device)))
...
class DanmakuModel(PreTrainedModel):
    ...
    """
    def __init__(self, config, position_embeddings):
        super(DanmakuModel, self).__init__(config)
        self.embeddings = BertEmbeddings(config, position_embeddings)
    ...
    # from_sequence是作为query，to_sequence是作为key和value，当两者相同时即为self-attention，这里就是self-attention
        # # 1.6 input_ids [batch_size, seq_len] 
        batch_size, time, h = input_ids.size()
        attention_mask = torch.ones((batch_size, time), dtype=torch.long).to(input_ids.device)
        ...
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]  # 取最后一层输出，[batch_size, seq_len, hidden_size]其实是[batch_size*time_step, max_words, hidden_size]
